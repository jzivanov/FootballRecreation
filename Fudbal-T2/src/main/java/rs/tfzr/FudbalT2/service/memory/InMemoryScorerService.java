...
public class InMemoryScorerService extends AbstractInMemoryService<Scorers> implements ScorerService
{
	...
	public List<Scorers> getRankList() 
	{
		...
		for(Scorers scorer: findAll())
		{
			...
			{
				if(scorer.getPlayer().getUser().getId() == scorer2.getPlayer().getUser().getId())
					num++;
			}
			...
			for(Scorers scorer2: map.keySet())
			{
				...
				&& scorer.getPlayer().getUser().getId() == scorer2.getPlayer().getUser().getId())
				{
					exists = true;
				}
			}
			if(!exists) map.put(scorer, num);
		}
		rankList.addAll(sortByComparator(map).keySet());
		return rankList;
	}
	
	private static Map<Scorers, Integer> sortByComparator(Map<Scorers, Integer> unsortMap) 
	{
		List<Map.Entry<Scorers, Integer>> list = 
			new LinkedList<Map.Entry<Scorers, Integer>>(unsortMap.entrySet());
		...
		for (Iterator<Map.Entry<Scorers, Integer>> it = list.iterator(); it.hasNext();) {
